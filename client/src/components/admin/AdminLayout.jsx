...
import { FiGrid, FiCalendar, FiPackage, FiDollarSign, FiSettings, FiLogOut, FiMenu, FiX, FiExternalLink, FiUser } from 'react-icons/fi';

const NAV = [
  { path: '/admin/dashboard', icon: FiGrid, label: 'Dashboard' },
  { path: '/admin/bookings', icon: FiCalendar, label: 'Reservas' },
  { path: '/admin/equipment', icon: FiPackage, label: 'Equipos' },
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-t border-gray-100 space-y-1">
        <a href="/" target="_blank" rel="noreferrer" className="flex items-center gap-3 px-4 py-2 rounded-xl text-sm text-gray-600 hover:bg-gray-100">
          <FiExternalLink size={16} /> Ver página pública
        </a>
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <aside className="relative flex flex-col w-72 bg-white shadow-xl z-50">
            <button onClick={() => setOpen(false)} className="absolute top-4 right-4 p-2 rounded-lg hover:bg-gray-100"><FiX size={20} /></button>
            <SidebarContent />
          </aside>
        </div>
      )}
      <main className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        <header className="lg:hidden bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-30">
          <button onClick={() => setOpen(true)} className="p-2 rounded-lg hover:bg-gray-100"><FiMenu size={20} /></button>
